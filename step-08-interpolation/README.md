...
> **補足:** このステップで実装するのは**補間（interpolation）のみ**です。
> 「予測（prediction / 外挿）」は概念として触れますが、コードには入れていません。
> 補間は「届いた2点の間」を埋める手法で、次の状態が来るまではゴール地点で止まって待ちます。
...
```
t = elapsed / duration
  = (今 - prevTime) / (nextTime - prevTime)
```

- `prevTime`: 補間区間の**開始時刻**（＝新しい状態を受け取った瞬間）
- `nextTime`: 補間区間の**終了時刻**（＝ `prevTime + interpolationDuration`）
- `duration`: 区間の長さ（＝ `interpolationDuration`、だいたい100ms）
- `elapsed`: 区間の開始からの経過時間（毎フレーム増えていく）
...
#### 次の更新が遅れたら？（`t` を 1.0 で止める）

実装では `t` を `Math.min(elapsed / duration, 1.0)` で計算しています。
つまり経過時間が `duration` を超えても、**`t` は 1.0 で頭打ち**になります。

```
| 経過時間 | t |
|  100ms | 1.00 |
|  150ms | 1.00 |  ← 次の状態がまだ来ない（クランプ）
|  200ms | 1.00 |  ← ゴール地点Bで止まって待つ
```

これにより、次の状態が遅れている間は **ゴール地点で止まって待ちます**。
`Math.min` が無いと `t` が 1.5、2.0 と増えてゴールを通り過ぎてしまいます（これが「予測 / 外挿」です）。

**このステップは「補間のみ」で、予測（外挿）は実装していません。**
次の状態が届いた瞬間に、止まっていた位置を新しいスタート地点にして、次のゴールへ動き出します。
